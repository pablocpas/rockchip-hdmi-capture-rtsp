...
write_config() {
  ...
  tmp="$(mktemp)"
  cat >"$tmp" <<EOF
# Configuration for rk-hdmi-streamer.service.
...
EOF
  if [ -n "${RGA_LIBRARY:-}" ]; then
    printf 'rga-library=%s\n' "$RGA_LIBRARY" >>"$tmp"
  else
    printf '# rga-library=/home/pi/librga/libs/Linux/gcc-aarch64/librga.so\n' >>"$tmp"
  fi
  cat >>"$tmp" <<EOF
width=$WIDTH
height=$HEIGHT
...
listen-rtsp=$LISTEN_RTSP
rtsp-path=$RTSP_PATH
max-clients=$MAX_CLIENTS
rtsp-debug=$RTSP_DEBUG
EOF
  if [ -n "${EXTRA_CONFIG_SECTIONS:-}" ]; then
    printf '%s\n' "$EXTRA_CONFIG_SECTIONS" >>"$tmp"
  else
    cat >>"$tmp" <<EOF

# Extra RTSP profiles are optional. Each profile inherits [main] and overrides
# only the keys listed in its section.
# [profile.720p50]
# path=/720p50
# stream-profile=rga
# width=1280
...
EOF
  ...
  if [ "$(id -u)" -eq 0 ] || [ -w "$(dirname "$out")" ]; then
    ...
    fallback="./rk-hdmi-streamer.conf.generated"
